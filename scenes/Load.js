class Load extends Phaser.Scene {
    constructor() {
        super("loadScene");
    }

    preload() {
        // set load path
        this.load.path = "assets/";
        // take care of all of our asset loading now
        this.load.image('groundScroll', 'ground.png');
        
        this.load.image('dragonGirl', 'dragongirl.png');
        this.load.image('background', 'background with moon and planet n stars.png');
        this.load.image('menu', 'menu screen.png');

        this.load.spritesheet('fly', 'Dragongirlspritesheet.PNG', {frameWidth:120, frameHeight:120, startFrame:0, endFrame:6});

        //load music
        this.load.audio('menuMusic', 'dragonLadyTheme.wav'); //divinty
        this.load.audio('playMusic', 'dragonLadyTheme.wav'); //forest fracas
        this.load.audio('gameOverMusic', 'dragonLadyTheme.wav'); //icarus theme

    }

    create() {
        //play music
        music = this.sound.add('menuMusic');
        music.play( {loop:true} );

        //add menu image
        this.add.image(0, 0, 'menu').setOrigin(0).setScale(.32, .3);

        //space bar is input
        spaceBar= this.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.SPACE);
    }

    update() {
        if (Phaser.Input.Keyboard.JustDown(spaceBar)) {
            //music.stop();
            this.scene.start("runnerScene");
        }
    }
}